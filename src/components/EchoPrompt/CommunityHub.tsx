      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Full prompts others saved — opens with the real generated text, not just empty fields.
          </p>
        </div>
            ...
            <Card key={prompt._id} className={`flex flex-col border-border/40 card-hover animate-slide-up stagger-${Math.min(i + 1, 6)}`}>
              ...
              <CardContent className="px-4 pb-4 pt-2 mt-auto space-y-3">
                ...
                {prompt._id && (
                  <StarRating
                    promptId={prompt._id}
                    initialRating={(prompt.ratingCount ?? 0) > 0 ? prompt.averageRating ?? 0 : 0}
                    ratingCount={prompt.ratingCount ?? prompt.analytics?.ratings?.length ?? 0}
                    ownerId={
